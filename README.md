...
- While inside the API pod run: `bundle exec rake db:seed; bundle exec rake db:create; bundle exec rake db:setup`.
...
- You may need to use:
  ...
  - `test` cluster: `pod_shell.sh test devstats devstats-postgres-0`: `pg_dump -Fc dev_analytics_test -f dev_analytics.dump`, `testk.sh -n devstats cp devstats-postgres-0:dev_analytics.dump dev_analytics.dump`, `prodk.sh -n devstats cp dev_analytics.dump devstats-postgres-0:dev_analytics.dump`
