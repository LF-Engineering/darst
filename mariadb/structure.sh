...
trap finish EXIT
"${1}k.sh" run --generator=run-pod/v1 --image=mariadb mariadb-structure --env="SH_HOST=`cat mariadb/secrets/HOST.secret`" --env="SH_USER=`cat mariadb/secrets/USER.secret`" --env="SH_PASS=`cat mariadb/secrets/PASS.${1}.secret`" --env="SH_DB=`cat mariadb/secrets/DB.secret`" -- /bin/sleep 3600s
