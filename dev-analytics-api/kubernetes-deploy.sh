...
vim --not-a-term -c "%s/image: .*/image: $DOCKER_USER\/dev-analytics-api/g" -c '%s/"bundle", "exec", "rails", "db:migrate"/"bundle", "exec", "rails", "db:migrate", "db:seed"/g' -c '%s/120/300/g' -c 'wq!' "${dd}/migrate.yml.erb"
